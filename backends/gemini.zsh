...
_ai_prompt_query_gemini_api() {
    ...
    local model="${ZSH_AI_PROMPT_MODEL:-${GEMINI_MODEL:-gemini-3.1-flash-lite-preview}}"
...
_ai_prompt_query_gemini() {
    ...
    if [[ -n "$api_key" ]]; then
        ...
        gemini -p "$prompt" -m "${ZSH_AI_PROMPT_MODEL:-${GEMINI_MODEL:-gemini-3.1-flash-lite-preview}}" -o text 2>/dev/null
    else
